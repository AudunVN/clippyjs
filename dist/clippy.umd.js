(function (global, factory) {
  ...
  var Queue = /*#__PURE__*/function () {
    ...
    _createClass(Queue, [{
      ...
      value: function progressQueue() {
        ...
        this.active = true; // execute function

        var completeFunction = cjq$4.proxy(this.next, this);
        f(completeFunction);
      }
    }, {
      key: "clear",
  ...
  var cjq$3 = jquery.noConflict();

  var Animator = /*#__PURE__*/function () {
    function Animator(el, path, data, sounds) {
      ...
      var curr = this.jElement;
      this.setupElement(this.jElement);

      for (var i = 1; i < this.data.overlayCount; i++) {
        var inner = this.setupElement(cjq$3('<div></div>'));
        curr.append(inner);
    ...
    _createClass(Animator, [{
      ...
      value: function setupElement(el) {
        var frameSize = this.data.framesize;
        el.css('display', "none");
        el.css({
          width: frameSize[0],
          height: frameSize[1]
      ...
      key: "preloadSounds",
      value: function preloadSounds(sounds) {
        for (var i = 0; i < this.data.sounds.length; i++) {
          var snd = this.data.sounds[i];
          var uri = sounds[snd];
          if (!uri) continue;
      ...
      key: "exitAnimation",
      value: function exitAnimation() {
        this.exiting = true;
      }
    }, {
      key: "showAnimation",
      value: function showAnimation(animationName, stateChangeCallback) {
        this.exiting = false;
      ...
      key: "getNextAnimationFrame",
      value: function getNextAnimationFrame() {
        if (!this.currentAnimation) return undefined; // No current frame. start animation.

        if (!this.currentFrame) return 0;
        var currentFrame = this.currentFrame;
        var branching = this.currentFrame.branching;
      ...
      value: function playSound() {
        var s = this.currentFrame.sound;
        if (!s) return;
        var audio = this.sounds[s];
        if (audio) audio.play();
      }
    }, {
      key: "atLastFrame",
      value: function atLastFrame() {
      ...
      value: function step() {
        ...
        var newFrameIndex = Math.min(this.getNextAnimationFrame(), this.currentAnimation.frames.length - 1);
        var frameChanged = !this.currentFrame || this.currentFrameIndex !== newFrameIndex;
        this.currentFrameIndex = newFrameIndex; // always switch frame data, unless we're at the last frame of an animation with a useExitBranching flag.
        ...
        this.draw();
        this.playSound();
        this.loop = window.setTimeout(cjq$3.proxy(this.step, this), this.currentFrame.duration); // fire events if the frames changed and we reached an end
  ...
  var cjq$2 = jquery.noConflict();

  var Balloon = /*#__PURE__*/function () {
    function Balloon(targetEl) {
    ...
    _createClass(Balloon, [{
      ...
      value: function setup() {
        this.balloon = cjq$2('<div class="clippy-balloon"><div class="clippy-tip"></div><div class="clippy-content"></div></div> ').hide();
        this.content = this.balloon.find('.clippy-content');
        cjq$2(document.body).append(this.balloon);
      }
    }, {
      key: "reposition",
      ...
      key: "position",
      value: function position(side) {
        var o = this.targetEl.offset();
        var h = this.targetEl.height();
        var w = this.targetEl.width();
        o.top -= cjq$2(window).scrollTop();
        o.left -= cjq$2(window).scrollLeft();
        var bH = this.balloon.outerHeight();
        var bW = this.balloon.outerWidth();
      ...
      value: function isOut() {
        var o = this.balloon.offset();
        var bH = this.balloon.outerHeight();
        var bW = this.balloon.outerWidth();
        var wW = cjq$2(window).width();
        var wH = cjq$2(window).height();
        var sT = cjq$2(document).scrollTop();
        var sL = cjq$2(document).scrollLeft();
        var top = o.top - sT;
        var left = o.left - sL;
        var m = 5;
      ...
      key: "speak",
      value: function speak(complete, text, hold) {
        this.hidden = false;
        this.show();
        var c = this.content; // set height to auto
      ...
      value: function hide(fast) {
        ...
        this.hiding = window.setTimeout(cjq$2.proxy(this.finishHideBalloon, this), this.CLOSE_BALLOON_DELAY);
      }
    }, {
      key: "finishHideBalloon",
      value: function finishHideBalloon() {
      ...
      key: "sayWords",
      value: function sayWords(text, hold, complete) {
        this.active = true;
        this.hold = hold;
        ...
        var el = this.content;
        var idx = 1;
        this.addWord = cjq$2.proxy(function () {
          if (!this.active) return;
          ...
          if (idx > words.length) {
            ...
            idx++;
            this.loop = window.setTimeout(cjq$2.proxy(this.addWord, this), time);
          }
        }, this);
        this.addWord();
      }
  ...
  var cjq$1 = jquery.noConflict();

  var Agent = /*#__PURE__*/function () {
    function Agent(path, data, sounds) {
      _classCallCheck(this, Agent);

      this.path = path;
      this.queue = new Queue(cjq$1.proxy(this.onQueueEmpty, this));
      this.jElement = cjq$1('<div class="clippy"></div>').hide();
      this.element = this.jElement[0];
      cjq$1(document.body).append(this.jElement);
    ...
    */
    ...
    _createClass(Agent, [{
      ...
      */
      ...
      key: "hide",
      value: function hide(fast, callback) {
        this.hidden = true;
        var el = this.jElement;
        this.stop();
      ...
      key: "moveTo",
      value: function moveTo(x, y, duration) {
        var dir = this.getDirection(x, y);
        var anim = 'Move' + dir;
        ...
        this.addToQueue(function (complete) {
          ...
          var callback = cjq$1.proxy(function (name, state) {
      ...
      key: "playInternal",
      value: function playInternal(animation, callback) {
        // if we're inside an idle animation,
        if (this.isIdleAnimation() && this.idleDfd && this.idleDfd.state() === 'pending') {
          this.idleDfd.done(cjq$1.proxy(function () {
      ...
      key: "play",
      value: function play(animation, timeout, cb) {
        ...
        this.addToQueue(function (complete) {
          ...
          if (timeout) {
            window.setTimeout(cjq$1.proxy(function () {
      ...
      */
      ...
      value: function show(fast) {
        ...
        if (this.jElement.css('top') === 'auto' || !this.jElement.css('left') === 'auto') {
          var left = cjq$1(window).width() * 0.8;
          var top = (cjq$1(window).height() + cjq$1(document).scrollTop()) * 0.8;
          this.jElement.css({
            top: top,
            left: left
          });
      ...
      */
      ...
      key: "speak",
      value: function speak(text, hold) {
        this.addToQueue(function (complete) {
      ...
      */
      ...
      value: function getDirection(x, y) {
        var offset = this.jElement.offset();
        var h = this.jElement.height();
        var w = this.jElement.width();
      ...
      */
      ...
      key: "onQueueEmpty",
      value: function onQueueEmpty() {
        if (this.hidden || this.isIdleAnimation()) return;
        var idleAnim = this.getIdleAnimation();
        this.idleDfd = cjq$1.Deferred();
        this.animator.showAnimation(idleAnim, cjq$1.proxy(this.onIdleComplete, this));
      }
    }, {
      key: "onIdleComplete",
      value: function onIdleComplete(name, state) {
        if (state === Animator.States.EXITED) {
      ...
      */
      ...
      key: "isIdleAnimation",
      value: function isIdleAnimation() {
        var c = this.animator.currentAnimationName;
      ...
      */
      ...
      value: function setupEvents() {
        cjq$1(window).on('resize', cjq$1.proxy(this.reposition, this));
        this.jElement.on('mousedown', cjq$1.proxy(this.onMouseDown, this));
        this.jElement.on('dblclick', cjq$1.proxy(this.onDoubleClick, this));
      }
    }, {
      key: "onDoubleClick",
      ...
      value: function reposition() {
        ...
        var o = this.jElement.offset();
        var bH = this.jElement.outerHeight();
        var bW = this.jElement.outerWidth();
        var wW = cjq$1(window).width();
        var wH = cjq$1(window).height();
        var sT = cjq$1(window).scrollTop();
        var sL = cjq$1(window).scrollLeft();
        var top = o.top - sT;
        var left = o.left - sL;
        var m = 5;
      ...
      value: function startDrag(e) {
        ...
        this.moveHandle = cjq$1.proxy(this.dragMove, this);
        this.upHandle = cjq$1.proxy(this.finishDrag, this);
        cjq$1(window).on('mousemove', this.moveHandle);
        cjq$1(window).on('mouseup', this.upHandle);
        this.dragUpdateLoop = window.setTimeout(cjq$1.proxy(this.updateLocation, this), 10);
      }
    }, {
      key: "calculateClickOffset",
      value: function calculateClickOffset(e) {
        var mouseX = e.pageX;
        var mouseY = e.pageY;
        var o = this.jElement.offset();
        return {
      ...
      key: "finishDrag",
      value: function finishDrag() {
        window.clearTimeout(this.dragUpdateLoop); // remove handles

        cjq$1(window).off('mousemove', this.moveHandle);
        cjq$1(window).off('mouseup', this.upHandle); // resume animations
      ...
      key: "addToQueue",
      value: function addToQueue(func, scope) {
        if (scope) func = cjq$1.proxy(func, scope);
  ...
  function load(name, successCb, failCb, base_path) {
    base_path = base_path || window.CLIPPY_CDN || 'https://gitcdn.xyz/repo/pi0/clippyjs/master/assets/agents/';
    var path = base_path + name;
    var mapDfd = load.map(path);
    var agentDfd = load.agent(name, path);
    var soundsDfd = load.sound(name, path);
    var data;
...
//# sourceMappingURL=clippy.umd.js.map
